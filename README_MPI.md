...
Another way that we explored to increase the speed ups on the OpenAcc optimized version is using MPI.
In a first implementation, we opted to use 2 nodes with 1 core and 1 GPU each, to gain some intuition about the techinical challenges
of the code.
The idea is to compute on a different GPU to compute a term on the Lindblad equation, separating the fist summation (summ over ), using MPI and OpenAcc.
