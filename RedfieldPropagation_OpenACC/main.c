...
// main method for running Redfiel integration
int main(void) {
	...
	// two dimensional, squared
	double *comm_real[SIZE], *comm_imag[SIZE], *lindblad_real[SIZE], *lindblad_imag[SIZE], *V[SIZE];
	for (i = 0; i < SIZE; i++){
	...
	for (step = 0; step < number_of_steps; step++) {
		...
		/* copy density matrix to host for readout */
		#pragma acc update host(rho_real[0:SIZE][0:SIZE])

		/* rotate back into site bases */
		transpose(eigVects, SIZE);
		rotate(rho_real, eigVects, SIZE);
		rotate(rho_imag, eigVects, SIZE);
		transpose(eigVects, SIZE);

		/* report current populations */
		printf("%d ", step);
        for (i = 0; i < SIZE; i++) {
