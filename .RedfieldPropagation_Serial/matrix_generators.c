...
// generates a matrix of size N x N with only ones
...
// generates a matrix of size N x N with randomly generated integers between 0 and 9
...
// generates a Hamiltonian matrix with randomly generated doubles from 0 and 999
...
// generates a complex identity matrix (real and complex components treated separately)
void gen_identity_complex(double *A_real, double *A_imag, int N) {
	gen_identity_real(A_real, N);
	gen_zero_matrix_real(A_imag, N);
}

// generates a complex zero matrix (real and complex components treated separately)
