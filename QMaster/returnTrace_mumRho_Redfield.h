#ifndef ReturnTrace_mumRho_Redfield_REDFIEDL_H
#define ReturnTrace_mumRho_Redfield_REDFIEDL_H

#include "headers.h"

// returns real and imaginary part of tr(mum rho(t)) as vectors

class ReturnTrace_mumRho_Redfield: public ManipulatingObservable
{
public:
   System *system;
...
public:
   ...
   ReturnTrace_mumRho_Redfield(System &INsystem, OpenCL_Init INOpenCLinfo, int INt_mod, Matrix *INmum, Matrix *INHelp, int INdelayTime); // returns the trace of the time evolution to the main programme
   void execute(double time, int it, cl_mem d_sigma_System, cl_mem d_sigma_help);
};

ReturnTrace_mumRho_Redfield::ReturnTrace_mumRho_Redfield(System &INsystem, OpenCL_Init INOpenCLinfo, int INt_mod, Matrix *INmum, Matrix *INHelp, int INdelayTime) :
...
void ReturnTrace_mumRho_Redfield::execute(double time, int it, cl_mem d_sigma_System, cl_mem d_sigma_help)
{
