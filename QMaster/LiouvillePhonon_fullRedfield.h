#ifndef LIOUVILLEPHONON_FULLREDFIELD_H
#define LIOUVILLEPHONON_FULLREDFIELD_H
...
class LiouvillePhonon_fullRedfield: public Liouville
{
public:
   System *system;
...
private:
   ...
   myreal hbardt;
public:
   LiouvillePhonon_fullRedfield(System &INsystem, OpenCL_Init &INOpenCLinfo);
   ~LiouvillePhonon_fullRedfield();
...
LiouvillePhonon_fullRedfield::LiouvillePhonon_fullRedfield(System &INsystem, OpenCL_Init &INOpenCLinfo) :
...
LiouvillePhonon_fullRedfield::~LiouvillePhonon_fullRedfield()
{
}

void LiouvillePhonon_fullRedfield::AddSite(int INsite, int INNDL, double *INlambda, double_complex *INgamma)
{
...
double_complex LiouvillePhonon_fullRedfield::Digamma(double_complex z)
{
...
double LiouvillePhonon_fullRedfield::phononStatistic(double energy)
{
   double mu=0;
   return 1./(exp(system->beta*(energy-mu))-1);
}

double LiouvillePhonon_fullRedfield::LDshifted(double lambda, double gamma, double shift, double omega)
{
...
double LiouvillePhonon_fullRedfield::SpectralDens(int j, double energy)   //FIXME what is j, not sites should come from listNDLsite
{
...
double LiouvillePhonon_fullRedfield::dSpectralDens(int j)
{
...
double_complex LiouvillePhonon_fullRedfield::gamma(int j, double energy)
{
...
void LiouvillePhonon_fullRedfield::createLambda(int j, Matrix *INLambda, Matrix *INLambdadag, Matrix* INVjdiag)
{
   int i=0;
   for(int M=0; M<system->Nsites; M++)
   {
      for(int N=0; N<system->Nsites; N++)
      {
         ...
         INLambda->assignEntry(M,N, c);
         INLambdadag->assignEntry(N,M, cconj);
         INVjdiag->assignEntry(M,N,cVjdiag);
      }
   }
}

void LiouvillePhonon_fullRedfield::initialize()
{
...
void LiouvillePhonon_fullRedfield::Liouv(Matrix *INrhoalt, Matrix *INrhonew, myreal dt)
{
...
void LiouvillePhonon_fullRedfield::execute(cl_myreal2* INsigmaold, cl_myreal2* INsigmanew, myreal dt)
{
...
void LiouvillePhonon_fullRedfield::freeDeviceMemory()
{
//	   listlambda.clear();
//	   listgamma.clear();
//	   listNDL.clear();
