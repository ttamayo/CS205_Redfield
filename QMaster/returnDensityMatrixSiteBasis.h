#ifndef RETURNDENSITYMATRIXSITEBASIS_H
#define RETURNDENSITYMATRIXSITEBASIS_H


#include "headers.h"

class returnDensityMatrixSiteBasis: public Observable
{
public:
   System *system;
...
private:
   ...
   double *h_rhoEntries;
public:
   returnDensityMatrixSiteBasis(System &INsystem,OpenCL_Init INOpenCLinfo, int INt_mod, int INNsteps);
   void execute(double time, int it,  cl_mem d_sigma_System);
};

returnDensityMatrixSiteBasis::returnDensityMatrixSiteBasis(System &INsystem, OpenCL_Init INOpenCLinfo,int INt_mod, int INNsteps) :
...
void returnDensityMatrixSiteBasis::execute(double time, int it,  cl_mem d_sigma_System)
{
   if(it%t_mod==0)
   {
      Ndata+=1;
