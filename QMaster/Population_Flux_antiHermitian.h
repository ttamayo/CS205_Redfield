/*
 * Population_Flux_antiHermitian.h
 *
 *  Created on: Dec 19, 2013
 *      Author: christoph
 */

#ifndef POPULATION_FLUX_ANTIHERMITIAN_H_
#define POPULATION_FLUX_ANTIHERMITIAN_H_

#include "headers.h"
using namespace::std;

class Population_Flux_antiHerm: public Observable
{
public:
   System *system;
...
public:
   Population_Flux_antiHerm(System &INsystem, OpenCL_Init INOpenCLinfo, int INt_mod, double INdt); // computes the time derivative of rho_00.
...
Population_Flux_antiHerm::Population_Flux_antiHerm(System &INsystem, OpenCL_Init INOpenCLinfo, int INt_mod, double INdt) :
...
void Population_Flux_antiHerm::compute_flux()
{
...
void Population_Flux_antiHerm::compute_branching_probability(int siteinit)
{
   for(int n=0; n<system->Nsites; n++)
   {
      for(int m=0; m<system->Nsites; m++)
...
void Population_Flux_antiHerm::get_dominant_flux_pathways(int siteinit, int target, double threshold)
{
...
void Population_Flux_antiHerm::execute(double time, int it, cl_mem d_sigma_System)
{
