...
//	for (i = 0; i < N; i++) 
//		printf("%.5f ", hamiltonian[i]);
//	printf("\n\n");

//	int unsigned idx;

	#pragma acc kernels  //present(hamiltonian[0:N])  present(rho_real[0:N*N], rho_imag[0:N*N])  //copyout(comm_real[0:N*N], comm_imag[0:N*N]) copyin(rho_real[0:N*N], rho_imag[0:N*N]) present(hamiltonian[0:N]) 
...
//	print_matrix_real(comm_real, N);
//	print_matrix_real(comm_imag, N);
//	exit(1);
...
void lindblad_operator(double * rho_real, double *rho_imag, double *gammas, double *eigVects, double *lindblad_real, double *lindblad_imag, double *links_to_loss, double *links_to_target, double *all_Vs, int SIZE) {
	...
	#pragma acc data copyin(rho_imag[0:SIZE*SIZE], rho_real[0:SIZE*SIZE]) present_or_copyin(all_Vs[0:3*SIZE*SIZE*SIZE]) create(V[0:SIZE*SIZE], helper[0:SIZE*SIZE], first_real[0:SIZE*SIZE], second_real[0:SIZE*SIZE]) copy(lindblad_real[0:SIZE*SIZE], lindblad_imag[0:SIZE*SIZE])
	for (m = 1; m < SIZE - 1; m++) {
...
//	#pragma acc data copy(lindblad_real[0:N*N], lindblad_imag[0:N*N])
