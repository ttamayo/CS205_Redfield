...
void matrix_mul_scalar(double *A, double scalar, int N) {
 	int unsigned i, j;
 	double temp;
	#pragma omp parallel shared(A,temp,scalar,N) private(i,j)
	#pragma omp for
	for (i = 0; i < N; i++) {
 		for (j = 0; j < N; j++) { 
			temp = A[j+i*N];
...
void matrix_add_real(double *A, double *B, double *C, int N) {
	int unsigned i, j;
	#pragma omp parallel shared(C,A,B,N) private(i,j)
	#pragma omp for
	for (i = 0; i < N; i++) 
		for (j = 0; j < N; j++) 
...
void matrix_mul_real(double *A, double *B, double *C, int N) {
	int unsigned i, j, k;
	double sum;
	#pragma omp parallel shared(A,B,C,N,sum) private(i,j)
	#pragma omp for
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			sum = 0.;
